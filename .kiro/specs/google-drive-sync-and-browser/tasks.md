...
- [ ] 1. **Escopo + OAuth do Drive** (Requisitos 1, 2). Adicionar o escopo de Drive a `SCOPES`
...
- [ ] 2. **Cliente da Drive API** (Requisitos 3, 7). Novo `src/googledrive.ts`: `files.list`
...
- [ ] 3. **Navegador de Drive (view nativa)** (Requisito 7). `DriveBrowserView extends
...
- [ ] 4. **Baseline_Drive + Delta_Cursor (persistência)** (Requisito 6). Campos/estrutura para
...
- [ ] 5. **Detecção bidirecional** (Requisito 3). Lado Drive via Changes API + Delta_Cursor;
...
- [ ] 6. **Admissão: decideAction + merge de 3 vias** (Requisito 4). Tabela de decisão
...
- [ ] 7. **Deleção segura** (Requisito 5). Edit-vence-deleção (ambiguidade → conflito);
...
- [ ] 8. **Execução + commit atômico** (Requisito 6). Aplicar só o plano autorizado; atualizar
...
- [ ] 10. **Testes + não-regressão** (Requisito 8). Testes de `decideAction` (matriz base/local/

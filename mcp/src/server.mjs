...
const tools = {
  ...
  list_savings_buckets: { description: "List savings buckets (piggy banks) with their derived balances. Always includes the fixed 'Emergency fund' bucket.", inputSchema: NONE, handler: async () => (await store.loadSavingsBuckets()).map((b) => ({ ...b, balance: Math.round(Object.values(b.log).reduce((a, v) => a + v, 0) * 100) / 100 })) },
  create_savings_bucket: { description: "Create a new custom savings bucket (e.g. Investments, Travel).", inputSchema: S.obj({ name: S.str(), goal: S.num("Optional target balance") }, ["name"]), handler: (a) => store.addSavingsBucket(a.name, a.goal) },
  update_savings_bucket: { description: "Rename a custom bucket and/or set its goal. The Emergency fund's name can't be changed, but its goal can.", inputSchema: S.obj({ id: S.str(), name: S.str(), goal: S.num() }, ["id"]), handler: (a) => store.updateSavingsBucket(a.id, { name: a.name, goal: a.goal }) },
  delete_savings_bucket: { description: "Delete a custom savings bucket by id (no-op for the Emergency fund).", inputSchema: S.obj({ id: S.str() }, ["id"]), handler: (a) => store.deleteSavingsBucket(a.id) },
  add_savings_contribution: { description: "Record a contribution (or withdrawal, via a negative amount) to a savings bucket on a given date.", inputSchema: S.obj({ id: S.str(), amount: S.num("Positive to contribute, negative to withdraw"), date: S.str("YYYY-MM-DD (defaults today)") }, ["id", "amount"]), handler: (a) => store.addSavingsContribution(a.id, a.amount, a.date) },

...
export class FitnessModule {
  ...
  private renderWorkoutPlan(root: HTMLElement, exercises: Exercise[]): void {
    ...
    splits.forEach((s) => {
      ...
      card.onclick = () => {
        ...
        else { this.selectedSplit = s.id; this.selectedDate = null; this.weightSplit = s.id; this.workoutActive = false; this.startTime = null; this.checked.clear(); }
        this.ctx.refresh();
      };
  ...
  private renderCalendar(root: HTMLElement, workouts: Workout[]): void {
    const splitByDay = new Map<string, string>();
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (sp) {
        ...
        cell.onclick = () => { this.selectedDate = ds; this.selectedSplit = null; this.ctx.refresh(); };
      }
  ...
  private renderDayDetail(root: HTMLElement, workouts: Workout[]): void {
    ...
    top.createEl("h3", { text: `🏋️ ${ds} — ${workout ? workout.exercises.length : 0} exercises`, cls: "pa-panel-title" });
    const close = top.createEl("button", { text: "✕", cls: "pa-icon-btn" });
    close.onclick = () => { this.selectedDate = null; this.ctx.refresh(); };

    if (!workout || !workout.exercises.length) { panel.createEl("p", { cls: "pa-muted", text: "No exercises logged this day." }); return; }
  ...
  private renderExerciseRow(tbody: HTMLElement, ex: Exercise): void {
    ...
    del.onclick = () => new ConfirmModal(this.ctx.app, `Delete exercise "${ex.name}"?`, async () => { await this.ctx.store.deleteExercise(ex); this.ctx.refresh(); }).open();
  }
